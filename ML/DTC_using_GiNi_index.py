# -*- coding: utf-8 -*-
"""
Created on Mon Apr 20 21:44:48 2020
...
dataset = pd.read_csv('Session_20//LR.csv')
...
plt.title('Classifier Deciasion Tree (Training set)')
plt.xlabel('Age')
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c = ListedColormap(('red', 'blue'))(i), label = j)
plt.title('Classifier Deciasion Tree(Test set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()
